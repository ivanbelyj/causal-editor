...
import { addDragAndScale } from "./add-view-drag-and-scale.js";

export function causalView(selector, idBasedData) {
  const { dag, implementationEdges } =
    getDagAndImplementationEdgesSet(idBasedData);
  const parentSelection = d3.select(selector);
  renderCausalView(parentSelection, dag, implementationEdges);
}
  ...
  const dag = d3
    ...
    .parentIds((node) => {
      ...
      if (abstractFactId) {
        ...
        implementationEdges.add(
          sourceAndTargetIdsToEdgeId(abstractFactId, node["Id"])
        );
      }
      return res;
    })(data);
...
function renderCausalView(svgParentSelection, dag, implementationEdges) {
  // const nodeRadius = 20;
  const nodeWidth = 140;
  const nodeHeight = 40;
  const layout = d3
  ...
  const width = dagWidth; // parseFloat(svgParentSelection.style("width"));
